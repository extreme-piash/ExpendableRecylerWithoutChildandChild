 ...
 */
public class RecylerviewAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public RecylerviewAdapter(Context context, List<BaseListModel> itemList) {
    ...
    public void onListTitleClicked(int position) {
        ...
        TitleModel titleModel = (TitleModel) itemList.get(position);

        if (titleModel.children.isEmpty()) {
            Toast.makeText(context, "Open activity for (Empty)" + titleModel.name, Toast.LENGTH_LONG).show();
